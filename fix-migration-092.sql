-- Fix for migration 092 - only add missing pieces
-- Run this in Supabase SQL Editor

-- Skip policies that already exist, only create tables and other objects

-- ============================================================================
-- 1. REVENUE CENTERS TABLE - only table and indexes (policies exist)
-- ============================================================================
...
-- Policies already exist, skip them

-- ============================================================================
-- 2. SERVICE PERIOD COVERS TABLE
-- ============================================================================
...
begin
  if not exists (
    ...
    where tablename = 'proforma_service_period_covers'
    and policyname = 'Users can view service period covers for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_service_period_covers'
    and policyname = 'Users can insert service period covers for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_service_period_covers'
    and policyname = 'Users can update service period covers for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_service_period_covers'
    and policyname = 'Users can delete service period covers for their scenarios'
  ) then
...
-- ============================================================================
-- 3. CENTER SERVICE PARTICIPATION TABLE
-- ============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_center_service_participation'
    and policyname = 'Users can view participation for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_center_service_participation'
    and policyname = 'Users can insert participation for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_center_service_participation'
    and policyname = 'Users can update participation for their scenarios'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where tablename = 'proforma_center_service_participation'
    and policyname = 'Users can delete participation for their scenarios'
  ) then
