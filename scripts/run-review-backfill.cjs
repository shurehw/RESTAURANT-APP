/**
 * Standalone Review Backfill Script (CommonJS)
 * Runs directly with Node.js — no Next.js build required.
 *
 * Usage: node scripts/run-review-backfill.cjs
 */
...
const TIPSEE_CONFIG = {
  host: process.env.TIPSEE_DB_HOST || 'tipsee-002.postgres.database.azure.com',
  user: process.env.TIPSEE_DB_USER || 'finance_HAggarwal_read_only',
