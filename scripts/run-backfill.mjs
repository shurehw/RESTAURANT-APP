...
const SUPABASE_SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1ucmFlZXNzY3FzYWFwcGthbGRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MjYzOTE3NCwiZXhwIjoyMDc4MjE1MTc0fQ.N4erm4GjP1AV8uDP2OVPBIdZnNtuofPmyLFdM2IVhXI';

const TIPSEE_CONFIG = {
  host: 'tipsee-002.postgres.database.azure.com',
  port: 5432,
  user: 'finance_HAggarwal_read_only',
  password: 'finance_HAggarwal_read_only_password',
  database: 'postgres',
...
async function syncVenueDay(venueId, tipseeLocationUuid, businessDate) {
  ...
  try {
    ...
    // Fallback: try old punches table if new_tipsee_punches has no data
    ...
    if (laborRow && parseInt(laborRow.punch_count) > 0) {
      // Calculate OT hours from new_tipsee_punches first, fallback to punches
      const otResult = await tipseePool.query(
        ...
        WHERE location_uuid = $1
          ...
          AND clocked_out IS NOT NULL
          AND NOT EXISTS (
            SELECT 1 FROM public.new_tipsee_punches ntp
            WHERE ntp.location_uuid = $1
              AND ntp.clocked_in::date = $2::date
              AND ntp.clocked_out IS NOT NULL
              AND ntp.is_deleted IS NOT TRUE
          )
        GROUP BY user_id
