-- ============================================================================
-- MIGRATION 288: Rez-Forecast Sanity Check
--
-- Problem: Poppy Feb 20 — confirmed=49, walkin_ratio=3.55 → pred=174,
-- but actual=121. The ratio has drifted from trained value.
-- DOW median (107) would be closer.
--
-- Fix: Only use rez-based prediction when it's within 2.5× of DOW median.
-- If the rez prediction is wildly different from history, fall back.
-- ============================================================================
...
base AS (
  SELECT
    ...
    -- Quality-gated walk-in pattern (min 5 samples, ratio <= 4.0)
...
SELECT
  ...
  -- Final prediction:
  --   Rez mode: confirmed × walk-in ratio, IF:
  --     1. T-24 snapshot exists
  --     2. Walk-in pattern quality gate passes (>=5 samples, ratio <=4.0)
  --     3. Sanity check: rez prediction is within 2.5× of DOW median
  ...
  CASE WHEN is_closed_day OR covers_raw = 0 THEN 0
    ...
    WHEN on_hand_resos IS NOT NULL
      ...
      AND on_hand_resos >= 3
      -- Sanity: rez prediction must be within 2.5× of DOW median
      AND covers_raw > 0
  ...
  -- rez_mode = rez prediction will actually be used (including sanity check)
...
SELECT 'forecasts_with_bias updated with rez sanity check (2.5x DOW median bound)' AS status;
