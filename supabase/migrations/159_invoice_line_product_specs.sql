...
ALTER TABLE invoice_lines
  ADD COLUMN IF NOT EXISTS catch_weight NUMERIC(12,3),           -- Actual billed weight (e.g., 29.40 LB)
  ADD COLUMN IF NOT EXISTS piece_count INTEGER,                   -- Number of pieces per case (e.g., 4 PC)
  ADD COLUMN IF NOT EXISTS nominal_case_weight NUMERIC(12,3),    -- Expected/nominal case weight (e.g., 28#)
  ADD COLUMN IF NOT EXISTS product_specs JSONB;                   -- Flexible storage for trim, grade, origin, etc.

COMMENT ON COLUMN invoice_lines.catch_weight IS 'Actual weight billed (for catch-weight items like proteins, seafood)';
COMMENT ON COLUMN invoice_lines.piece_count IS 'Number of pieces per case (e.g., "4 PC" in tenderloin case)';
...
CREATE INDEX IF NOT EXISTS idx_invoice_lines_product_specs ON invoice_lines USING GIN (product_specs);
