-- Update create_invoice_with_lines to support parsed_pack and normalized_description fields
-- These fields improve matching and item creation from OCR data
...
BEGIN
  ...
  -- Insert Invoice Lines (with product specs, parsed pack, and normalized description)
  ...
  LOOP
    INSERT INTO invoice_lines (
      ...
      NULLIF(line_item->>'normalized_description', ''),
      (line_item->>'quantity')::NUMERIC,
      (line_item->>'unit_cost')::NUMERIC,
      (line_item->>'ocr_confidence')::NUMERIC,
      CASE
        WHEN line_item->'parsed_pack' IS NOT NULL AND line_item->'parsed_pack' != 'null'::jsonb
        THEN line_item->'parsed_pack'
        ELSE NULL
      END,
      ...
      CASE
        WHEN line_item->'product_specs' IS NOT NULL AND line_item->'product_specs' != 'null'::jsonb
...
COMMENT ON FUNCTION create_invoice_with_lines IS 'Creates invoice with lines including parsed pack configuration, normalized description, and product specifications';
