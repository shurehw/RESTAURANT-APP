-- ============================================================================
-- MIGRATION 287: Walk-In Pattern Quality Gate
--
-- Problem: Keys and Poppy have unreliable walk-in patterns (low sample size,
-- extreme ratios). Rez mode with a ratio of 17x produces worse predictions
-- than the DOW median fallback.
--
-- Fix: Only activate rez mode when:
--   1. Pattern has >= 5 samples (sufficient training data)
--   2. Ratio <= 4.0 (reservations account for >= 25% of covers)
--      Venues where most covers are walk-ins should NOT use rez mode.
-- ============================================================================
...
base AS (
  SELECT
    ...
    -- Only include walk-in pattern if quality gate passes
    CASE WHEN wp.sample_size >= 5 AND wp.median_ratio <= 4.0
...
SELECT
  ...
  -- Final prediction:
  --   Rez mode: confirmed × validated walk-in ratio (quality-gated)
  --   Fallback: DOW median × pacing + bias
  CASE WHEN is_closed_day OR covers_raw = 0 THEN 0
    WHEN on_hand_resos IS NOT NULL
      AND walkin_ratio IS NOT NULL     -- quality gate applied above
      AND on_hand_resos >= 3
  ...
  -- Rez mode active when quality gate passes
  (on_hand_resos IS NOT NULL AND walkin_ratio IS NOT NULL AND on_hand_resos >= 3) AS rez_mode,
  bias_corrected, bias_reason, holiday_code, holiday_adjustment, venue_class,
  covers_lower, covers_upper
FROM base;

SELECT 'forecasts_with_bias updated with quality gate (min_samples=5, max_ratio=4.0)' AS status;
