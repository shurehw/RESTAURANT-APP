      ...
      {showVenueSelector ? (
        <Popover open={venueOpen} onOpenChange={setVenueOpen}>
          <PopoverTrigger asChild>
            <button
              type="button"
              className="flex items-center gap-2 px-2.5 py-1 border border-keva-slate-300 rounded-md bg-white hover:bg-keva-slate-50 hover:shadow-sm transition-all text-sm text-foreground font-medium"
            >
              <Building2 className="w-4 h-4 text-keva-slate-500" />
              <span>{selectedVenue?.name || 'Select venue'}</span>
              <ChevronDown className="w-3.5 h-3.5 text-keva-slate-400" />
            </button>
          </PopoverTrigger>
        ...
        <div className="flex items-center gap-2 px-2.5 py-1 text-sm text-foreground font-medium">
          <Building2 className="w-4 h-4 text-keva-slate-500" />
          {venues[0].name}
        </div>
      ) : null}
