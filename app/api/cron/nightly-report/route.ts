...
import { runNightlyAgentsForVenues, type OrchestratorResult } from '@/lib/agents/orchestrator';
...
async function processOrg(
  ...
  // 5. Run all nightly agents → Action Center (before email send)
  if (process.env.ANTHROPIC_API_KEY) {
    const agentContexts: AgentContext[] = orgVenues
      ...
      .map((v) => ({
        ...
        labor: laborCache.get(v.id),
        tipseeUuid: tipseeMappings.get(v.id),
      }));

    const agentResults = await runNightlyAgentsForVenues(agentContexts);
    const totalActions = agentResults.reduce((sum, r) => sum + r.totalActions, 0);
    if (totalActions > 0) {
      console.log(`[nightly-report-cron] Agents created ${totalActions} action(s) across ${agentResults.length} venue(s)`);
    }
  }
