...
async function fetchOpenCheckSummariesFromItems(
  ...
  const result = await pool.query(
    `SELECT
      ci.check_id as id,
      'Open Tab' as table_name,
      'Unknown' as employee_name,
      0 as guest_count,
      COALESCE(SUM(ci.price::numeric * ci.quantity::numeric), 0) as sub_total,
      COALESCE(SUM(ci.price::numeric * ci.quantity::numeric), 0) - COALESCE(SUM(ci.comp_total::numeric), 0) - COALESCE(SUM(ci.void_value::numeric), 0) as revenue_total,
      COALESCE(SUM(ci.comp_total::numeric), 0) as comp_total,
      COALESCE(SUM(ci.void_value::numeric), 0) as void_total,
      MIN(ci.date) as open_time,
      NULL as close_time,
      true as is_open,
      0 as payment_total,
      0 as tip_total
